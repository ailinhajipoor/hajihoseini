...
class NotesListviewsTest(TestCase):
    def setUp(self):
        self.note_obj = Notes.objects.create(author="khodam", text="sample text.")

    def test_notes_list_views_url(self):
        response = self.client.get('/')
        # boro be browser  va ba client (masalan karbar dare darkhast mide)va get request befrest va javab ro bargardoon
        self.assertEqual(response.status_code,200)
        # barresi mikone ke jaavb barabar ba 200 hast ya na  ! 200 yani dardastres hastesh ya na
